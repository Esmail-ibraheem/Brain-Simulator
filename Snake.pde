class Snake{
    ...
    public Snake(int layers){
        ...
        if(!humanPlaing){
            ...
            brain = new NeuralNetwork(24, hiddenLayers, 4, layers);
    ...
    public Snake(ArrayList<Food>foods){
        ...
        decision = new float[4];
        ArrayList<PVector>body = new ArrayList<PVector>();
        ArrayList<Food>foodList = new ArrayList<Food>(foods.size()) ;
        for(Food f:foods){
    ...
    public Snake crossOver(Snake partnerSnake){
        Snake childSnake = new Snake(hiddenLayers);
        childSnake.brain = brain.crossOver(partnerSnake.brain) ;
        return childSnake;
    }
    ...
    public void move_Snake() {  
        if(!dead){
            ...
            if(wallCollide(head.x,head.y)) {
                ...
                dead = true;
            } else if(moveLeft <= 0 && !humanPlaying) {
                dead = true;
            }
        }
    }
    ...
    public void shift_Body() {  //make the body follow the head 
        float tempx = head.x;
        float tempy = head.y;
        head.x += xVelocity;
        head.y += yVelocity;
    ...
    public float[] look_in_Directions(PVector direction){
        ...
        float distance = 0
        ArrayList<PVector>pos = new ArrayList<PVector>(head.x, head.y) ;
        pos.add(direction);
        distance++ ;
