class NeuralNetwork{
    ...
    public NeuralNetwork(int inputNodes, int hiddenNodes, int outputNodes, int hiddenLayers){
        ...
        weights[weights.length-1] = new Matrix(outputNodes, hiddenNodes) ;
    ...
    public float[] activation_Function_Output(float[] inputArray){
        ...
        for(int i=0; i<hiddenLayers; i++){
            Matrix hiddenLayer_input = weights[i].dot_Product(currentBias) ;
            Matrix hiddenLayer_output = hiddenLayer_input.activation_Function() ;
            currentBias = hiddenLayer_output.add_Bias() ;
        }

        Matrix outputLayer_input = weights[weights.length-1].dot_Product(currentBias);
        Matrix output = outputLayer_input.activation_Function() ;

        return output.convert_to_Array() ;
    }
    public NeuralNetwork crossOver(NeuralNetwork partnerNeuraNetwork){
        NeuralNetwork childNeuralNetwork = new NeuralNetwork(inputNodes, hiddenNodes, outputNodes, hiddenLayers);
        for(int i=0; i<weights.length; i++){
            childNeuralNetwork.weights[i] = weights[i].crossOver(partnerNeuraNetwork.weights[i]);
        }
    ...
    public void neuralNetwork_Visualizer(float xPosition, float yPosition, float width, float height, float[]vision, float[]decision){
        ...
        for(int i=0; i<weights[0].rows; i++){
            for(int j=0; i<weights[0].colunms-1; j++){
